...
var referer_patt3 = new RegExp("^http:\/\/[^\.]+\.getgvod\.com\/");
			...
			if(0 == bttype && suburl)
			{
				...
				if(subUrlext == ".avi" || subUrlext == ".mkv" || subUrlext == ".mp4")
				{
					if ( v_gvods > 0 && ( v_gvods < VGVODS_1_5 ) )
					{
						...
						hint_stat(1);
						genDivStr("http://js.getgvod.com/player/enforceupdate.htm");
						return;
					}
				}
			}			
...
function updateGVOD(queryStr){
	...
	genDivStr("http://js.getgvod.com/player/update.htm?"+queryStr+"&hostname="+location.hostname);
}
function fInstallGVOD(versionDetector){
	hint_stat(11);
	var pLctn = document.URL;
	genDivStr("http://js.getgvod.com/player/finstall.htm");
}
...
function hint_stat(stat_value){
	var URL = "http://stat.getgvod.com/fcg-bin/fcgi_gvod_stat.fcg?stat_key=1&stat_value="+stat_value+"&domain="+location.hostname;
	pgvByImg(URL);
}
function genDivStr(src){  
    ...
    $('#m dd.l i').html("<iframe id = 'iframe_proxy' name='iframe_proxy' scrolling='no' frameborder='0' style='margin:0; width:720px;height:480px;' src='"+src+"' ></iframe>");
}
function noGVOD(){
...
function notIE(){
	var ieWarnPage = '/i/gvod.htm';
	hint_stat(0);
	genDivStr(ieWarnPage);
}
function showEInstall(){
	genDivStr("/i/gvod.htm");
}
